...
class DensoVP6242(DHRobot):  # pylint: disable=too-many-ancestors
    ...
    def __init__(self):
        ...
        dh_table = [
            RevoluteDH(d=0.125, a=0, alpha=np.pi/2, offset=0, qlim=self.qlims[0]),
            RevoluteDH(d=0, a=0.210, alpha=0, offset=np.pi/2, qlim=self.qlims[1]),
            RevoluteDH(d=0, a=-0.075, alpha=-np.pi/2, offset=-2*np.pi/2, qlim=self.qlims[2]),
            RevoluteDH(d=0.210, a=0, alpha=np.pi/2, offset=0, qlim=self.qlims[3]),
            RevoluteDH(d=0, a=0, alpha=-np.pi/2, offset=0, qlim=self.qlims[4]),
            RevoluteDH(d=0.07, a=0, alpha=0, offset=0, qlim=self.qlims[5])
        ]
